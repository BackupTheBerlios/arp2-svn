...
cache_destroy (void)
{
  ...
  for( i = 0, bitmap = (RD_HBITMAP) g_volatile_bc;
       (size_t) i < sizeof( g_volatile_bc ) / sizeof( RD_HBITMAP );
       ++i, ++bitmap )
  {
    if( *bitmap != NULL )
    {
  ...
  for( i = 0, cursor = (RD_HCURSOR*) g_cursorcache;
       (size_t) i < sizeof( g_cursorcache ) / sizeof( RD_HCURSOR );
       ++i, ++cursor )
  {
    if( *cursor != NULL )
    {
