...
//  else if ( (servaddr.sin_addr.s_addr = inet_addr(server)) == -1 )
	...
	if (bind
	    (l_sock, (struct sockaddr *) &servaddr,
	...
	setsockopt(l_sock, SOL_SOCKET, SO_REUSEADDR, (void *) &true,
		   sizeof (true));

	setsockopt(l_sock, IPPROTO_TCP, TCP_NODELAY, (void *) &true,
		   sizeof (true));
