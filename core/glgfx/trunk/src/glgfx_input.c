...
//	  if (ev->altmask is true
	...
	int dx = ev->x_root - monitor->mouse_x;
	int dy = ev->y_root - monitor->mouse_y;
	...
	if (dx != 0 || dy != 0) {
	  XWarpPointer(monitor->display, None, RootWindow(monitor->display,
							  monitor->vinfo->screen),
		       0, 0, 0, 0,
		       monitor->mode.hdisplay / 2, monitor->mode.vdisplay / 2);

	  // Make sure the event generated by this call results in dx=dy=0, so we don't 
	  // end up calling XWarpPointer forever.
	  monitor->mouse_x = monitor->mode.hdisplay / 2;
	  monitor->mouse_y = monitor->mode.vdisplay / 2;
	}
	break;
      }
    }
