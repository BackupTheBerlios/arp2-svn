...
#if DEBUG_DRIVE_ID
static const char *drive_id_name (const drive *drv)
{
    switch (drv->drive_id) {
...
#ifdef DISK_DEBUG2
    write_dlog ("%08.8X %02.2X %s", m68k_getpc (&regs), data, tobin(data));
#endif
...
void DSKDAT (uae_u16 v)
{
    ...
    if (count < 5) {
	count++;
	write_dlog ("%04.4X written to DSKDAT. Not good. PC=%08.8X", v, m68k_getpc (&regs));
	if (count == 5)
