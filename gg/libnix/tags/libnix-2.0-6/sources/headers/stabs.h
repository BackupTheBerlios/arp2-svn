...
/* no weak alias support -- use a normal alias instead */
#define WEAK_ALIAS(a,b) ALIAS(a,b)
...
/* linker can use symbol b for symbol a if a is not defined */
#define ALIAS(a,b) asm(".set " #a "," #b "; .globl " #a  );

/* a weak alias */
#define WEAK_ALIAS(a,b) asm(".weak " #a " ; " #a " = " #b);

/* add symbol a to list b (type c (22=text 24=data 26=bss) -- currently ignored) */
#define ADD2LIST(a,b,c) asm(".section .libnix_" #b "\n" \
                            ".long " #a "\n" \
                            ".text \n ")

/* Install private constructors and destructors pri MUST be -127<=pri<=127 */
#define ADD2INIT(a,pri) ADD2LIST(a,__INIT_LIST__,22); \
                        ADD2LIST(pri+128,__INIT_LIST__,22)
#define ADD2EXIT(a,pri) ADD2LIST(a,__EXIT_LIST__,22); \
                        ADD2LIST(pri+128,__EXIT_LIST__,22)
 ...
 * define a as a label for an absolute address b
 */
#define ABSDEF(a,b) asm( #a "=" #b ";.globl " #a )
...
#define ADDTABL_END() asm(".stabs \"__FuncTable__\",20,0,0,-1")
