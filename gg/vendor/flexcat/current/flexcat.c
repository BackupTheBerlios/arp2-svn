...
char EString[] = "E-mail: carlos@amiga.com.pl or marcin@orlowski.com WWW: http://amiga.com.pl/flexcat/";
//|
...
void CreateCat(char *CatFile)
{
  ...
  if(CatVersionString)
	...
	if( found )
	  {
	  if(verStr = malloc(strlen(cc.ChunkStr)+128))
		{
		char dateStr[10];
	  ...
	  if(found1 && found1[1]=='(' && isdigit(found1[2]))
		...
		if(found2)
		  ...
		  if(verStr = malloc(strlen(cc.ChunkStr)+128))
			...
			sprintf(verStr, "%s%s%s", cc.ChunkStr, dateStr, found2);
			free(cc.ChunkStr);
			cc.ChunkStr = verStr;
			}
		  else
