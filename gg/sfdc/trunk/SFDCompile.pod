...
gatestubs or gateproto files. Possible values are B<none>, B<first>
and B<last>. The default is B<none>.
...
Generate a F<SAS/C>/F<LATTICE>/F<DICE> pragmas file.
...
similar modules. F<sfdc> can also generate F<SAS/C> and F<DICE> files
for end users of such modules.
...
Generate F<gcc> B<inlines> (actually preprocessor macros) or F<SAS/C>
...
F<Include/clib/> file and either the F<gcc> inlines or F<SAS/C>
pragmas.
