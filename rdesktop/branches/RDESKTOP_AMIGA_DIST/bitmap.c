...
BOOL
bitmap_decompress8(unsigned char *output, int width, int height,
		  unsigned char *input, int size);
BOOL
bitmap_decompress16(unsigned char *output, int width, int height,
		  unsigned char *input, int size);
		  ...
		  unsigned char *input, int size)
{
	...
	int lastopcode = -1, insertmix = False, bicolour = False;
	uint8 code;
	uint16 colour1 = 0, colour2 = 0, mix = 0xffff;
	...
	while (input < end) {
		...
		case 0xf:
			opcode = code & 0xf;
			// UHM... 0xD was 0xB in 8bit decomp.
			// anyone with WindowsXP who can confirm this is correct?
			count = (opcode < 9) ? CVAL((uint16*)input): (opcode < 0xD)? 8:1;
			offset = 0;
			break;
		...
		case 0:	/* Fill */
			...
			break;
		case 8:	/* Bicolour */
			colour1 = CVAL((uint16*)input);
		case 3:	/* Colour */
			colour2 = CVAL((uint16*)input);
			break;
		case 6:	/* SetMix/Mix */
		case 7:	/* SetMix/FillOrMix */
			mix = CVAL((uint16*)input);
			opcode -= 5;
			break;
		case 9:	/* FillOrMix_1 */
			mask = 0x03;
		...
		while (count > 0) {
			...
			case 4:	/* Copy */
				REPEAT(line[x] = CVAL((uint16*)input));
				break;
