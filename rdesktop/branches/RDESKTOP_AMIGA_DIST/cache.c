...
cache_destroy (void)
{
  ...
  for( i = 0, font = (FONTGLYPH*) fontcache;
       i < sizeof( fontcache ) / sizeof( FONTGLYPH );
       ++i, ++font )
  {
    if( font->pixmap != NULL )
    {
  ...
  for( i = 0, text = (DATABLOB*) textcache;
       i < sizeof( textcache ) / sizeof( DATABLOB );
       ++i, ++text )
  {
    if( text->data != NULL )
    {
  ...
  for( i = 0, cursor = (HCURSOR*) cursorcache;
       i < sizeof( cursorcache ) / sizeof( HCURSOR );
       ++i, ++cursor )
  {
    if( *cursor != NULL )
    {
