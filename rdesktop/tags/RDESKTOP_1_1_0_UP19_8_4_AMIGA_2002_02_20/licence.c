		     ...
		     char *user, char *host)
{
	...
	uint16 userlen = (user == 0 ? 0 : strlen(user) + 1);
	uint16 hostlen = (host == 0 ? 0 : strlen(host) + 1);
	...
	/* default 0xff010000 */
	/* w2k client 0x03010000 */
	out_uint32_le(s, (built_in_license ? 0x03010000 : 0xff010000) );
