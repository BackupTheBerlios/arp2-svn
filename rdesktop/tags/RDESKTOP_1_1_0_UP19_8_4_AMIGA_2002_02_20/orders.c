...
#ifdef GUI_WIN32
	 ui_line(os->opcode, os->startx, os->starty,
...
process_memblt(STREAM s, MEMBLT_ORDER * os, uint32 present, BOOL delta)
{
	...
	colmap = cache_get_colourmap(os->colour_table);
	if (colmap == NULL)
 		return;

	ui_memblt(ROP2_S(os->opcode), os->x, os->y, os->cx, os->cy,
	    bitmap, colmap, os->srcx, os->srcy);
}
...
process_text2(STREAM s, TEXT2_ORDER * os, uint32 present, BOOL delta)
{
	...
	DEBUG
	    ("TEXT2(x=%d,y=%d,cl=%d,ct=%d,cr=%d,cb=%d,bl=%d,bt=%d,bb=%d,br=%d,fg=0x%x,bg=0x%x,font=%d,fl=0x%x,mix=%d,unk=0x%x,n=%d)\n",
	     ...
	     os->boxbottom, os->fgcolour, os->bgcolour, os->font, os->flags,
	     os->mixmode, os->unknown, os->length);
...
process_colcache(STREAM s)
{
	...
	hmap = ui_create_colourmap(&map);
        cache_put_colourmap(cache_id, hmap);
...
process_fontcache(STREAM s)
{
	...
	for (i = 0; i < nglyphs; i++) {
		...
		/* XXX, investigate if it's possible to get rid of "m"..  */
		m = 0;

		while (j) {
			unroll8Expr( rev_data[m++] = rotate(data[m]); )
			    j -= 8;
		}
		j = (datasize & 0x7);
		if (j)
			switch (j) {
				unroll8Lefts( rev_data[m++] = rotate(data[m]); )
			}
...
#endif
		cache_put_font(font, character, offset, baseline, width, height,
			       bitmap);
	}
...
process_orders(STREAM s)
{
		...
		if (order_flags & RDP_ORDER_SECONDARY) {
			...
			if (order_flags & RDP_ORDER_BOUNDS) {
				...
				ui_set_clip(os->bounds.left,
					    os->bounds.top,
					    os->bounds.right - os->bounds.left +
					    1,
					    os->bounds.bottom - os->bounds.top +
					    1);
			}
			...
			case RDP_ORDER_DESTBLT:
				process_destblt(s, &os->destblt, present,
						delta);
				break;
			...
			case RDP_ORDER_SCREENBLT:
				process_screenblt(s, &os->screenblt, present,
						  delta);
				break;
			...
			case RDP_ORDER_DESKSAVE:
				process_desksave(s, &os->desksave, present,
						 delta);
				break;
			...
			case RDP_ORDER_POLYLINE:
				process_polyline(s, &os->polyline, present,
						 delta);
				break;
	...
	// not true since we introduced decompress.c
	//if (s->p != next_packet)
...
reset_order_state()
{
	...
	/* Must set initial order (fix for coloured backgrounds on logon screen) */
